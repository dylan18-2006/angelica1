...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 09-03-2026 a las 17:05:43
...
CREATE TABLE `audi_autor` (
  ...
  `AUT_APELLIDO_AUDI` varchar(20) DEFAULT NULL,
  `AUT_MUERTE_AUDI` date DEFAULT NULL,
  `AUT_NACIMIENTO_AUDI` date DEFAULT NULL
...
(1, 13, 'DILAN', 'MUÑOZ', 'CALLE123', '31762354', 'DILAN', 'MUÑOZ', 'calle 72 #2', '3167027241', '2026-03-05 07:24:15', 'root@local', 'Actualización'),
(2, 13, 'DILAN', 'MUÑOZ', 'calle 72 #2', '3167027241', NULL, NULL, NULL, NULL, '2026-03-05 08:12:35', 'root@local', 'Registro eliminado');
...
--
DELIMITER $$
CREATE TRIGGER `autor_delete` AFTER DELETE ON `autor` FOR EACH ROW BEGIN
...
(1, 'ana luna', 'luna@gmail.com'),
(2, 'dylan', 'dylan@gmail.com');

--
-- Disparadores `cliente`
--
DELIMITER $$
CREATE TRIGGER `SEGUIMIENTO_CLIENTE` BEFORE UPDATE ON `cliente` FOR EACH ROW INSERT INTO LOG_CLIENTE(
    ID_CLIENTE,
    NOMBRE_ANTERIOR,
    CORREO_ANTERIOR,
    NOMBRE_NUEVO,
    ...
    COMENTARIO)
    VALUES (NEW.ID_CLIENTE,OLD.NOMBRE_CLIENTE,OLD.CORREO,NEW.NOMBRE_CLIENTE,NEW.CORREO,NOW(),USER(),'SE ATUALIZO DATOS')
$$
DELIMITER ;
DELIMITER $$
...
--
-- Estructura Stand-in para la vista `cliente_v`
...
(0, 'Cien años de soledad', 'Novela', 417, 0),
...
--
-- Estructura Stand-in para la vista `libross`
-- (Véase abajo para la vista actual)
--
...
CREATE TABLE `log_cliente` (
  ...
  `CORREO_ANTERIO` varchar(20) DEFAULT NULL,
...
--
-- Estructura de tabla para la tabla `log_libro`
--

CREATE TABLE `log_libro` (
  `ID_LOG_LIBRO` int(11) NOT NULL,
  `LIB_TITULO_N` varchar(255) DEFAULT NULL,
  `LIB_NUM_PAGINA_N` int(11) DEFAULT NULL,
  `LIB_GENERO_N` varchar(100) DEFAULT NULL,
...
--
-- Volcado de datos para la tabla `log_libro`
--

INSERT INTO `log_libro` (`ID_LOG_LIBRO`, `LIB_TITULO_N`, `LIB_NUM_PAGINA_N`, `LIB_GENERO_N`, `FECHA`, `USUARIO`, `ACCION`) VALUES
(1, 'Cien años de soledad', 417, 'Novela', '2026-03-05 10:22:28', 'root@localhost', 'SE INSERTÓ UN LIBRO');
...
--
-- Estructura Stand-in para la vista `prestamo_cliente`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `prestamo_cliente` (
`nombre_cliente` varchar(20)
,`correo` varchar(255)
,`lib_cop_isbn` bigint(20)
...
--
-- Estructura para la vista `cliente_v`
--
DROP TABLE IF EXISTS `cliente_v`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `cliente_v`  AS SELECT `cliente`.`nombre_cliente` AS `NOMBRE_CLIENTE`, `cliente`.`correo` AS `CORREO` FROM `cliente` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `libross`
--
DROP TABLE IF EXISTS `libross`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `libross`  AS SELECT `libro`.`LIB_TITULO` AS `LIB_TITULO`, `libro`.`LIB_GENERO` AS `LIB_GENERO`, `libro`.`LIB_DIASPRESTAMO` AS `LIB_DIASPRESTAMO` FROM `libro` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `libro_cliente`  AS SELECT `cliente`.`nombre_cliente` AS `NOMBRE_CLIENTE`, `cliente`.`correo` AS `CORREO`, `libro`.`LIB_DIASPRESTAMO` AS `LIB_DIASPRESTAMO`, `libro`.`LIB_TITULO` AS `LIB_titulo`, `libro`.`LIB_GENERO` AS `lib_genero` FROM (`cliente` join `libro`) ;

-- --------------------------------------------------------

--
-- Estructura para la vista `prestamo_cliente`
--
DROP TABLE IF EXISTS `prestamo_cliente`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `prestamo_cliente`  AS SELECT `cliente`.`nombre_cliente` AS `nombre_cliente`, `cliente`.`correo` AS `correo`, `prestamo`.`LIB_COP_ISBN` AS `lib_cop_isbn`, `prestamo`.`PRES_FECHA_PRESTAMO` AS `pres_fecha_prestamo`, `prestamo`.`PRES_FECHA_DEVOLUCION` AS `pres_fecha_devolucion` FROM (`cliente` join `prestamo`) ;
...
--
-- Indices de la tabla `log_libro`
--
ALTER TABLE `log_libro`
  ADD PRIMARY KEY (`ID_LOG_LIBRO`);
...
--
-- AUTO_INCREMENT de la tabla `log_libro`
--
ALTER TABLE `log_libro`
  MODIFY `ID_LOG_LIBRO` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
...
-- Eventos
--
CREATE DEFINER=`root`@`localhost` EVENT `anual_eliminar_prestamos` ON SCHEDULE EVERY 1 YEAR STARTS '2026-03-09 07:55:39' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
DELETE FROM tbl_prestamo
WHERE pres_fechaDevolucion <= NOW() - INTERVAL 1 YEAR;
#datos menores a la fecha actual - 1 año
END$$

CREATE DEFINER=`root`@`localhost` EVENT `ELIMINAR_PRESTAMOS` ON SCHEDULE EVERY 1 YEAR STARTS '2026-03-09 10:21:26' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
	DELETE FROM prestamo
    WHERE PRE_FECHA_DEVOLUCION <= NOW() - INTERVAL 1 YEAR ;
